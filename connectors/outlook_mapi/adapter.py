...
# The connector is intentionally outside the search-service package.  Add the
# sibling service source directory only so this standalone CLI can reuse the
# service's canonical local paths and request-size limit.
WORKSPACE_ROOT = Path(__file__).resolve().parents[2]
SERVICE_SOURCE_DIRECTORY = WORKSPACE_ROOT / "service"
if str(SERVICE_SOURCE_DIRECTORY) not in sys.path:
    sys.path.insert(0, str(SERVICE_SOURCE_DIRECTORY))
...
OWNED_RUN_PREFIX = "outlook-mapi-import-"
OWNERSHIP_MARKER_NAME = ".ragsearch-outlook-mapi-import-owned"


class ImportFailure(RuntimeError):
    """A safe, user-facing Outlook MAPI connector failure."""


class ImportCancelled(RuntimeError):
    """The caller requested a graceful Outlook MAPI import cancellation."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to the built OutlookMapiReader.exe",
    )
    parser.add_argument(
        "--service-url",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Outlook MAPI import subdirectory is created underneath it."
        ),
    )
    parser.add_argument(
        "--cancel-file",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Scan all stores, folders and messages (passes zero/unlimited reader limits); "
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "scan, 4000000 for --full-scan)"
        ),
    )
    parser.add_argument(
        "--store-contains",
        help="Optional OutlookMapiReader store display-name filter",
    )
    return parser


def validate_reader_executable(path: Path) -> Path:
    try:
        resolved = Path(path).resolve(strict=True)
    except (OSError, RuntimeError) as exc:
        raise ImportFailure(f"OutlookMapiReader executable does not exist: {path}") from exc
    if not resolved.is_file():
        raise ImportFailure(f"OutlookMapiReader executable is not a file: {resolved}")
    if resolved.name.casefold() != "outlookmapireader.exe":
        raise ImportFailure(
            f"OutlookMapiReader executable must have its canonical name: {resolved}"
        )
    return resolved
...
def _create_owned_spool_run(base_dir: Path) -> OwnedSpoolRun:
    ...
    try:
        ...
        raise ImportFailure(
            f"Could not create a private Outlook MAPI connector spool directory: {exc}"
        ) from exc
    ...
    if run_dir.parent != base_dir or not _is_strict_descendant(run_dir, base_dir):
        try:
            ...
            pass
        raise ImportFailure(
            "Private Outlook MAPI connector spool directory escaped the configured spool"
        )
    ...
    try:
        ...
        try:
            ...
            pass
        raise ImportFailure(
            f"Could not mark the private Outlook MAPI connector spool directory: {exc}"
        ) from exc
...
def _cleanup_owned_spool_run(owned: OwnedSpoolRun) -> None:
    ...
    try:
        ...
        raise ImportFailure(
            "Could not resolve the private Outlook MAPI connector spool during cleanup"
        ) from exc
    if (
        ...
        raise ImportFailure(
            "Refusing to clean a changed or unowned Outlook MAPI connector spool path"
        )
    ...
    try:
        ...
        raise ImportFailure(
            "Refusing to clean an Outlook MAPI connector spool without its ownership marker"
        ) from exc
    if not stat.S_ISREG(marker_mode) or marker.is_symlink() or not secrets.compare_digest(
        marker_value, owned.marker_token
    ):
        raise ImportFailure(
            "Refusing to clean an Outlook MAPI connector spool with an invalid ownership marker"
        )
    ...
    try:
        ...
        raise ImportFailure(
            f"Could not clean the private Outlook MAPI connector spool directory: {exc}"
        ) from exc


def _safe_reader_attachment_path(raw_path: str, spool_dir: Path) -> Path:
    if not raw_path or len(raw_path) > MAX_TEMP_PATH_CHARS or "\0" in raw_path:
        raise ImportFailure("OutlookMapiReader attachment temp_path is invalid")
    try:
        candidate = Path(raw_path).resolve(strict=True)
        mode = candidate.stat().st_mode
    except (OSError, RuntimeError) as exc:
        raise ImportFailure(
            f"OutlookMapiReader attachment temp_path does not exist: {raw_path}"
        ) from exc
    if not _is_strict_descendant(candidate, spool_dir) or not stat.S_ISREG(mode):
        raise ImportFailure(
            "OutlookMapiReader attachment temp_path is outside the private connector "
            "spool or is not a regular file"
        )
    return candidate
...
def _check_cancel(cancel_file: Path | None) -> None:
    ...
    if cancelled:
        raise ImportCancelled("Outlook MAPI import cancelled")
...
def _reader_text(
    ...
    *,
    ...
    if not isinstance(value, str):
        raise ImportFailure(f"OutlookMapiReader JSONL field {name!r} must be a string")
    if required and not value.strip():
        raise ImportFailure(f"OutlookMapiReader JSONL field {name!r} must not be empty")
    if len(value) > maximum_chars:
        raise ImportFailure(
            f"OutlookMapiReader JSONL field {name!r} exceeds {maximum_chars} characters"
        )
    if "\0" in value:
        raise ImportFailure(
            f"OutlookMapiReader JSONL field {name!r} contains a null character"
        )
    return value


def _reader_timestamp(record: Mapping[str, object], name: str) -> str | None:
    value = record.get(name)
    if value is None:
        return None
    if not isinstance(value, str) or not value or len(value) > MAX_TIMESTAMP_CHARS:
        raise ImportFailure(
            f"OutlookMapiReader JSONL field {name!r} must be an ISO-8601 string or null"
        )
    try:
    ...
    if "T" not in value or parsed.tzinfo is None or parsed.utcoffset() is None:
        raise ImportFailure(
            f"OutlookMapiReader JSONL field {name!r} must include a date, time, and UTC offset"
        )
    return value


def _reader_bool(record: Mapping[str, object], name: str) -> bool:
    value = record.get(name)
    if type(value) is not bool:
        raise ImportFailure(f"OutlookMapiReader JSONL field {name!r} must be a boolean")
    return value


def _reader_attachment(
    raw: object,
    *,
    ...
    if not isinstance(raw, dict):
        raise ImportFailure(
            f"OutlookMapiReader JSONL attachments[{attachment_index}] must be an object"
        )
    name = _reader_text(
        raw,
        "name",
    ...
    content_type = _reader_text(
        raw,
        "content_type",
        required=False,
        maximum_chars=MAX_CONTENT_TYPE_CHARS,
    )
    temp_path = _reader_text(
        raw,
        "temp_path",
        required=False,
    ...
    if (
        ...
        raise ImportFailure(
            f"OutlookMapiReader JSONL attachments[{attachment_index}].size must be a bounded "
            "non-negative integer"
        )

    safe_path: Path | None = None
    if temp_path:
        safe_path = _safe_reader_attachment_path(temp_path, spool_dir)
        try:
            actual_size = safe_path.stat().st_size
        except OSError as exc:
            raise ImportFailure(
                f"Could not stat OutlookMapiReader JSONL attachments[{attachment_index}].temp_path"
            ) from exc
        if actual_size > maximum_attachment_bytes:
            raise ImportFailure(
                f"OutlookMapiReader JSONL attachments[{attachment_index}] exceeds the spool "
                "file limit"
            )
...
def map_reader_message(
    ...
    *,
    ...
    if not isinstance(record, dict):
        raise ImportFailure("Each OutlookMapiReader JSONL record must be an object")
    ...
    if len(body) > maximum_body_chars:
        raise ImportFailure("OutlookMapiReader emitted a body larger than the requested limit")
    body_available = _reader_bool(record, "body_available")
    body_truncated = _reader_bool(record, "body_truncated")
    if not body_available and body:
        raise ImportFailure(
            "OutlookMapiReader JSONL body must be empty when body_available is false"
        )
    if body_truncated and not body_available:
        raise ImportFailure(
            "OutlookMapiReader JSONL body cannot be truncated when it is unavailable"
        )

    raw_attachments = record.get("attachments")
    if not isinstance(raw_attachments, list):
        raise ImportFailure("OutlookMapiReader JSONL field 'attachments' must be an array")
    if len(raw_attachments) > MAX_ATTACHMENTS_PER_MESSAGE:
        raise ImportFailure(
            "OutlookMapiReader JSONL field 'attachments' exceeds "
            f"{MAX_ATTACHMENTS_PER_MESSAGE} items"
        )
    attachments: list[dict[str, object]] = []
    for index, raw_attachment in enumerate(raw_attachments):
        attachment, _ = _reader_attachment(
            raw_attachment,
    ...
    return {
        ...
        "subject": subject,
        "sender_name": _reader_text(
            record,
            "sender_name",
            required=False,
            maximum_chars=MAX_METADATA_FIELD_CHARS,
        ),
        "sender_email": _reader_text(
            record,
            "sender_email",
            required=False,
            maximum_chars=MAX_METADATA_FIELD_CHARS,
        ...
        "sent_at": _reader_timestamp(record, "sent_at"),
        "received_at": _reader_timestamp(record, "received_at"),
        "modified_at": _reader_timestamp(record, "modified_at"),
        "internet_message_id": _reader_text(
            record,
            "internet_message_id",
            required=False,
            maximum_chars=MAX_METADATA_FIELD_CHARS,
        ),
        "conversation_id": _reader_text(
            record,
            "conversation_id",
            required=False,
            maximum_chars=MAX_METADATA_FIELD_CHARS,
...
def post_message(
    ...
    *,
    ...
    if len(encoded) > MAX_HTTP_REQUEST_BYTES:
        raise ImportFailure(
            "Outlook MAPI connector message exceeds the local HTTP request safety limit"
        )
    ...
    if (
        ...
        raise ImportFailure("RAGSearch service rejected the Outlook MAPI connector message")
...
def import_outlook_mapi_messages(
    *,
    executable: Path,
    service_url: str,
    token_path: Path,
    ...
    if full_scan:
        ...
        reader_max_stores = reader_max_folders = reader_max_messages = 0
    else:
        for name, value, maximum in numeric_limits:
            if type(value) is not int or not 1 <= value <= maximum:
                raise ImportFailure(f"{name} must be between 1 and {maximum}")
        reader_max_stores = max_stores
        reader_max_folders = max_folders
        reader_max_messages = max_messages
    ...
    executable = validate_reader_executable(executable)
    ...
    imported = 0
    bodies_truncated = 0
    progress_total = 0 if full_scan else int(reader_max_messages)
    ...
    try:
        ...
        command = [
            ...
            "--jsonl",
            "--max-stores",
            str(reader_max_stores),
            "--max-folders",
            str(reader_max_folders),
            "--max-messages",
            str(reader_max_messages),
            "--body-preview-chars",
        ...
        _report_progress(
            ...
            message="Starting Outlook MAPI scan",
        )
        try:
            process = popen(
                command,
                stdout=subprocess.PIPE,
                stderr=None,  # OutlookMapiReader diagnostics remain visible without buffering.
                text=True,
                encoding="utf-8",
                errors="strict",
                bufsize=1,
            ...
            raise ImportFailure(f"Could not start OutlookMapiReader.exe: {exc}") from exc
        if process.stdout is None:
            _stop_process(process)
            raise ImportFailure("Could not capture OutlookMapiReader JSONL output")
        ...
        while True:
            ...
            try:
                ...
                raise ImportFailure("OutlookMapiReader stdout is not valid UTF-8") from exc
            if not line:
                break
            output_line += 1
            if len(line) > maximum_line_chars:
                raise ImportFailure("OutlookMapiReader emitted an oversized JSONL record")
            if not line.strip():
                continue
            if reader_max_messages and imported >= reader_max_messages:
                raise ImportFailure("OutlookMapiReader emitted more messages than requested")
            try:
                reader_record = json.loads(line)
            except json.JSONDecodeError as exc:
                raise ImportFailure(
                    f"OutlookMapiReader emitted invalid JSONL at output line {output_line}"
                ) from exc
            message = map_reader_message(
                reader_record,
            ...
            try:
                ...
                imported += 1
                if reader_record["body_truncated"]:
                    bodies_truncated += 1
                _report_progress(
                    progress,
                    phase="importing",
        ...
        reader_exit_code = process.wait()
        _report_progress(
            progress,
            phase="complete" if reader_exit_code == 0 else "failed",
            current=imported,
            total=imported if reader_exit_code == 0 else progress_total,
            message=(
                f"Imported {imported} message(s); truncated bodies: {bodies_truncated}"
                if reader_exit_code == 0
                else f"OutlookMapiReader exited with code {reader_exit_code}"
            ),
            bodies_truncated=bodies_truncated,
        )
        return ImportResult(
        ...
        _report_progress(
            ...
            current=imported,
            total=progress_total,
            message="Outlook MAPI import cancelled",
            bodies_truncated=bodies_truncated,
        )
        raise
        ...
        _report_progress(
            ...
            current=imported,
            total=progress_total,
            message="Outlook MAPI import failed",
            bodies_truncated=bodies_truncated,
        )
        raise
    finally:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = import_outlook_mapi_messages(
        ...
        print("Outlook MAPI import cancelled", file=sys.stderr)
        return 130
    except ImportFailure as exc:
        print(f"Outlook MAPI import failed: {exc}", file=sys.stderr)
        return 1
    except KeyboardInterrupt:
        print("Outlook MAPI import interrupted", file=sys.stderr)
        return 130

    if result.reader_exit_code != 0:
        print(
            "OutlookMapiReader exited with code "
            f"{result.reader_exit_code} after importing {result.imported} message(s); "
            f"truncated bodies: {result.bodies_truncated}",
            file=sys.stderr,
        )
        return result.reader_exit_code
    print(
        f"Imported {result.imported} message(s) from OutlookMapiReader; "
        f"truncated bodies: {result.bodies_truncated}"
    )
    return 0
