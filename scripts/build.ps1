...
param(
    ...
    [string]$VstoConfiguration = 'Debug',
...
if (-not $SkipNative) {
    $nativeProject = Join-Path $workspace 'native-mapi-probe\NativeMapiProbe.vcxproj'
    Invoke-MSBuild @(
        $nativeProject,
        "/t:$Target",
        '/m',
...
if (-not $SkipVsto) {
    if ($VstoConfiguration -eq 'Release' -and
        [string]::IsNullOrWhiteSpace($CertificateThumbprint)) {
        throw 'Release VSTO builds require an explicit production CertificateThumbprint; the automatic development certificate is Debug-only.'
    }
    ...
    if (-not [string]::IsNullOrWhiteSpace($CertificateThumbprint)) {
        ...
        if ($normalizedThumbprint -notmatch '^[0-9A-F]+$') {
            throw 'CertificateThumbprint must contain hexadecimal characters only.'
        }
    ...
    Invoke-MSBuild @(
        ...
        "/t:$Target",
        '/m',
        "/p:Configuration=$VstoConfiguration",
